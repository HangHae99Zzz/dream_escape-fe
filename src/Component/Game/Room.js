...
import { ClickSound } from '../../Asset/Sound/mp3_sound';
...
export default function Model({
    ...
    useFrame(state => {
        ...
        refAa.current.rotation.z = (1 + Math.sin(t)) / 4;
    ...
    const modalHandler = quizType => {
        if (quizType === 'Bb' && count < countLimit - 1) {
            window.alert(
                `${
                    countLimit - 1 - count
                } 개의 남은 퀴즈를 해결하고 도전해주세요`
            );
            return;
        } else if (quizType === 'Bb' && count === countLimit) {
            // 문까지 맞춘 상태에서 다른 팀원이 문을 클릭했을 때
            setGamePassed(true);
            setGameEnd(true);
            return;
        }
