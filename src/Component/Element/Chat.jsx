...
const Chat = () => {
  ...
  const getDevices = async () => {
    try {
      divicesRef.current = await navigator.mediaDevices.enumerateDevices();
  ...
  const createPeerConnection = useCallback((socketID, email) => {
    try {
      ...
      pc.ontrack = (e) => {
        ...
        dispatch(roomActions.refPeers(roomId, socketRef.current.id, navigate));
  ...
  useEffect(() => {
    ...
    socketRef.current.on("user_exit", (data) => {
      ...
      // peers 업데이트
      dispatch(
        roomActions.refPeers(roomInfo.roomId, socketRef.current.id, navigate)
      );
          ...
          <select>
            {speakersRef.current?.map((speaker, i) => {
              return curSpeakerRef.label === speaker.label ? (
                <option value={speaker.deviceId} key={i} selected>
                  {speaker.label}
                </option>
              ) : (
